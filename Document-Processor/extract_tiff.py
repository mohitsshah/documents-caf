...
import argparse
import shutil
from modules.tiffProcessor import Reader
import glob
    ...
    for root, directories, filenames in os.walk(src):
        for f in filenames:
            if f.endswith(".tiff") or f.endswith(".tif"):
                files.append((f.split(".")[0], f.split(".")[1], root.replace(src, "").lstrip("/"), os.path.join(root, f)))
    return files


def process_file(item, dst, overwrite, tessdata, tessdata3):
    filename, ext, dirs, src = item
    dst_dir = os.path.join(dst, dirs, filename)
    if not os.path.exists(dst_dir):
        os.makedirs(dst_dir, exist_ok=True)
    dst_file = os.path.join(dst_dir, filename + "." + ext)
    if os.path.exists(os.path.join(dst_dir, filename + ".json")) and not overwrite:
        return False
    shutil.copy(src, dst_file)
    reader = Reader(
        params={"src": dst_dir, "name": filename, "overwrite": overwrite, "tessdata": tessdata, "tessdata3": tessdata3, "ext": ext})
...
def run(args):
    ...
    files = get_files(src)
    print("Found %d TIFF files in %s" % (len(files), src))
    for f in files:
...
if __name__ == '__main__':
    flags = argparse.ArgumentParser("Command line arguments for TIFF Document Processing")
